...
describe("Feature 2: Model Definitions", () => {
  ...
  describe("bootstrap model catalog", () => {
    ...
    it("uses image input for Claude and verified Luna, text input for other concrete bootstrap models", () => {
      const imageModels = kiroModels.filter((model) => model.id.startsWith("claude-") || model.id === "gpt-5-6-luna");
      const textOnlyModels = kiroModels.filter(
        (model) => !model.id.startsWith("claude-") && model.id !== "gpt-5-6-luna" && model.id !== "auto",
      );
